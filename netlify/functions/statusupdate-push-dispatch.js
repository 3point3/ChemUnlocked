/* =====================================================
   statusupdate-push-dispatch.js — Netlify Scheduled Function
   ...
   hash of the user's email and entryId identifies a specific check-in;
   neither belongs in logs to diagnose a delivery problem that counts
   alone can explain.
   ===================================================== */

const webpush = require('web-push')
const { connectLambda } = require('@netlify/blobs')
const { suStore } = require('./statusupdate-lib/session')
...
// never deliver a reminder more than this far past its due time.
const MAX_ATTEMPTS = 5
// Deliberately longer than MyMentals' 30 minutes, because the reminder
// means something different here. There, a "how are you feeling now?"
// follow-up referred to a specific moment and was worse than useless once
// that moment had passed. A daily check-in nudge has no such anchor — it
// is just as useful an hour or two late as it was on the minute, so the
// cap only needs to stop a backlog from firing overnight.
const MAX_LATENESS_MS = 2 * 60 * 60 * 1000 // 2 hours
...
exports.handler = async function (event) {
  ...
  const queueStore = suStore('push-queue')
  const subsStore = suStore('push-subs')
  ...
  for (const { key } of blobs) {
    ...
    // Past the window, give up rather than deliver something stale — and,
    // more to the point, rather than let a backlog built up during an
    // outage fire all at once, possibly in the middle of the night.
    ...
    if (!subs.length) {
      // The single most likely reason a reminder "never fires": it was
      // queued, but this account never completed the "Enable push
      // reminders" step on any device (or did so only in a browser
      // context that has since been cleared), so there is nowhere to
      // deliver it. Nothing to retry against — drop it.
      noSubscription++
      await queueStore.delete(key)
      continue
    }

    // Deep-links to a fresh check-in rather than to the queued entryId:
    // unlike MyMentals, this app has no per-entry follow-up screen to
    // return to. The entryId stays in the queue record because the
    // scheduling endpoint's contract carries it, but nothing here needs
    // it. Notification text says nothing about symptoms — it lands on a
    // lock screen.
    const payload = JSON.stringify({
      title: 'Daily check-in',
      body: 'Ready when you are.',
      url: '/statusupdate/daily/new',
    })
    ...
    for (const sub of pending) {
      try {
        ...
        if (err.statusCode === 404 || err.statusCode === 410) {
          ...
          failed++
          console.error('[statusupdate-push-dispatch] send failed:', err.statusCode || err.message)
        }
      }
    }
  ...
  if (due > 0) {
    console.log(
      `[statusupdate-push-dispatch] ${due} due: ${sent} sent, ${failed} failed, ` +
