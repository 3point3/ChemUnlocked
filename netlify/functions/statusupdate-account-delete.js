/* =====================================================
   statusupdate-account-delete.js — Netlify Serverless Function
   ...
   Only ever touches statusupdate-* stores (see statusupdate-lib/session.js,
   where every name is built from one prefix), so deleting a StatusUpdate
   account cannot reach a MyMentals account that happens to share an
   email — the two are separate accounts by design.
...
exports.handler = async function (event) {
  ...
  for (const name of ['entries', 'vaults', 'push-subs']) {
    try { await suStore(name).delete(accountId) } catch { /* already absent */ }
  }

  // Stores keyed by an opaque token/id whose value carries accountId.
  await deleteMatching('sessions', v => v && v.accountId === accountId)
  await deleteMatching('push-queue', v => v && v.accountId === accountId)

  // The account record itself.
  try { await suStore('accounts').delete(accountId) } catch { /* ignore */ }
