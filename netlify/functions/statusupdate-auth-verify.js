/* =====================================================
   statusupdate-auth-verify.js — Netlify Serverless Function
   ...
   Expected POST body (JSON), either:
     token                — the token from the emailed link, or
     email + code         — the code from that same email, typed in
   ===================================================== */

const { connectLambda } = require('@netlify/blobs')
const {
  json, redeemMagicLink, redeemSignInCode, getOrCreateAccount, createSession, createHandoff, suStore,
} = require('./statusupdate-lib/session')
...
exports.handler = async function (event) {
  ...
  const vault = await suStore('vaults').get(account.id, { type: 'json' })
