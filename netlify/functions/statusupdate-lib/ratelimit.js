...
/**
 * Takes back the most recent hit — used when the email itself failed to
 * send, so a provider hiccup doesn't make the person wait a minute (or
 * burn one of their five) for nothing. Best effort, fails quietly.
 */
async function forgive(store, key, now) {
  try {
    const record = await store.get(key, { type: 'json' })
    const hits = ((record && record.hits) || []).slice()
...
async function checkSignInLimits(store, emailHash, ip, now = Date.now()) {
  ...
  return { ok: true, undo: () => Promise.all([ip ? forgive(store, `ip:${sha256(ip)}`, now) : null, forgive(store, `email:${emailHash}`, now)]) }
}

module.exports = { decide, hit, forgive, clientIp, checkSignInLimits, EMAIL_POLICY, IP_POLICY }
