(function () {
  ...
  var BANK = window.MCAT_QUESTIONS || [];
  var ROUND = 10;
  var LETTERS = ["A", "B", "C", "D", "E"];
  var app = document.getElementById("app");

  // ---------- persistent stats ----------
  var STORE = "noa_mcat_prep_v1";
  function loadStore() {
  ...
  function renderStart() {
    ...
    if (!BANK.length) {
      app.innerHTML = '<p class="lead">Question bank is empty right now — add questions to ' +
        '<code>questions.js</code> to start practicing.</p>';
      return;
    }

    app.innerHTML =
      '<p class="lead">A bank of <strong>' + BANK.length +
      ' MCAT practice questions</strong>. Each round gives you ' + Math.min(ROUND, BANK.length) +
      ' questions, one at a time. Pick an answer to see whether it’s right — and why — then move on. ' +
      'A score appears after every round.</p>' +
      '<div class="stat-row">' +
      ...
      '</div>' +
      '<button class="btn" id="start">Start ' + Math.min(ROUND, BANK.length) + ' questions</button>' +
  ...
  function renderQuestion() {
    ...
    var pct = (s.pos / total) * 100;
    var html =
      '<div class="qbar">' +
        '<span class="meta">Question ' + (s.pos + 1) + ' of ' + total + '</span>' +
  ...
  function submitAnswer() {
    ...
    primary.textContent = (s.pos + 1 >= s.items.length) ? "See results" : "Next question";
  ...
  function finishRound() {
    ...
    var line, sub;
    if (pct >= 90) { line = "Outstanding."; sub = "You’re testing at a strong, exam-ready level."; }
    else if (pct >= 70) { line = "Solid work."; sub = "Comfortably in a competitive range — keep the streak going."; }
    else if (pct >= 50) { line = "Getting there."; sub = "Review the rationales you missed and run another set."; }
    else { line = "Keep practicing."; sub = "Read each rationale carefully — repetition is how this sticks."; }

    app.innerHTML =
      '<div class="result-head">' +
        '<div class="ring" style="--p:' + pct + '">' +
          '<div class="inner"><b>' + s.score + '/' + total + '</b><span>' + pct + '%</span></div>' +
        '</div>' +
      '</div>' +
      ...
      '<div class="actions">' +
        ...
        '<button class="btn" id="again">Try another round</button>' +
      '</div>';
